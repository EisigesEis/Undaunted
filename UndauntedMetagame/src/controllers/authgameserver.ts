...
const GameserverKeys = CreateAPIKeyCache("Gameserver", async () => {
    const Rows = await GetDb().query.gameserverapikeys.findMany({
        columns: {
            keyHash: true
        }
    });
...
export async function IsValidGameserverAPIKey(GameserverAPIKey: string){
    return await GameserverKeys.find(GameserverAPIKey) === true;
}
