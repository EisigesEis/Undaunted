...
const PurchaseTokenCleanupIntervalMs = 60_000;

export function PruneExpiredPurchaseTokens(Now = Date.now()) {
    let Removed = 0;
    for(const [Token, Payload] of IssuedPurchaseTokens){
        if(Payload.exp < Now){
            IssuedPurchaseTokens.delete(Token);
            Removed++;
        }
    }
    return Removed;
...
const PurchaseTokenCleanup = setInterval(() => PruneExpiredPurchaseTokens(), PurchaseTokenCleanupIntervalMs);
PurchaseTokenCleanup.unref();
