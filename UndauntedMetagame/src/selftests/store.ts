...
export async function runSelftest(){
    ...
    const {CreatePurchaseToken, GetBalancesForUser, GetLadyLuckCatalog, GetStoreCatalog, LadyLuckCatalog, PruneExpiredPurchaseTokens, PurchaseFromToken} = await import("../controllers/store");
    const Db = GetDb();
    ...
    const ExpiringToken = CreatePurchaseToken("store-user", "ladyluck_cell_trials_06_e", "markssteel");
    assert(ExpiringToken);
    const ExpiringPayload = JSON.parse(Buffer.from(ExpiringToken, "base64url").toString("utf8"));
    assert(PruneExpiredPurchaseTokens(ExpiringPayload.exp + 1) >= 1);
    assert.deepStrictEqual(await PurchaseFromToken("store-user", ExpiringToken, "markssteel"), {success: false, error: "invalid_token"});
